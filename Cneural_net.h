...
void back_propagate(struct neural_net *net, double learning_rate){
...
void forward_propagate(struct neural_net *net){
    ...
    // so put it through the sigmoid function
...
double transfer_tanh(double activation){
    return 2.0 / (1.0 + (2*exp(-activation))) - 1.0;
}
